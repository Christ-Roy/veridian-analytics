# TICKET — Passer analytics-engine en GitOps Nomad propre
...
> de analytics-engine vit DANS ce repo (gitops), tourne en **job Nomad** (plus de compose/Dokploy), et
> respecte les règles strictes de l'infra. **Le terrain a raison** : vérifie l'état réel avant d'agir.

> ⚠️ **Spécifique Analytics Engine** : deux backends de données. (1) Le **bridge Postgres** → cible cluster **Patroni** comme les autres apps (§4). (2) **ClickHouse** porte l'analytique lourde : sa mise en HA est un chantier SÉPARÉ (réplication CH / keeper), **hors de ce ticket** — ne la bascule pas en Patroni (ça n'a pas de sens), traite-la à part et note-la comme suite.

## Contexte (déjà fait par l'infra)
- Migration Dokploy → **cluster Nomad 3 nœuds** TERMINÉE. analytics-engine tourne DÉJÀ en job Nomad
  (`~/nomad-veridian/jobs/analytics-engine.nomad.hcl`, déployé, sain). Dokploy est décommissionné.
...
1. **Le job Nomad de analytics-engine vit dans CE repo** (ex `deploy/analytics-engine.nomad.hcl`), source de vérité gitops —
...
3. **Job conforme aux règles infra** (NON négociables) :
   - **Réseau privé par défaut** : tout port en `host_network = "tailscale"` (bind Tailscale-only).
     **JAMAIS `0.0.0.0` / port public arbitraire** — un hook harness bloque déjà `docker -p 0.0.0.0`.
     Seul l'ingress (Traefik, 80/443) est public ; analytics-engine est joignable UNIQUEMENT via l'ingress.
   ...
   - **Secrets** : Nomad Variable `nomad/jobs/analytics-engine` + `template{env=true}`. JAMAIS en clair dans le HCL/repo.
...
4. **DB en HA (si analytics-engine a une base Postgres)** : cible = cluster **Patroni** (failover auto via Consul),
   ...
   Coordonne avec l'infra / Robert pour la fenêtre si analytics-engine est critique/client-payant.

## Comment procéder
1. Lis `~/nomad-veridian/CLAUDE.md` + `jobs/templates/README.md` + le job actuel `jobs/analytics-engine.nomad.hcl`.
